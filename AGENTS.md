...
**FSEvents(watchdog)가 넘겨주는 경로의 파일명은 디스크에 NFC로 저장돼 있어도 NFD로 온다.** 이 문자열만 믿고 "NFD니까 바꿔야 한다"고 판단하면, 이미 NFC인 파일에 no-op rename을 걸고 그 rename이 또 이벤트를 만들어 **무한 루프**가 된다.

과거 실측: 유휴 상태에서 초당 230여 건의 rename/이벤트가 영원히 반복(CPU 8%, 감쇠 없음). 로그는 한 줄도 안 남아 눈치채기 어렵다.

그래서 `Handler.on_any_event`는 이벤트 경로를 **`resolve_stored_path()`로 실제 저장된 이름으로 되돌린 뒤** 처리한다. 이 호출을 빼면 루프가 그대로 돌아온다.

`os.walk`가 주는 이름은 실제 저장된 이름이라 안전하다 — 문제는 이벤트 경로뿐이다.

수정 후 실측(번들, NFD 파일 1개 변환 후 활동 없음): 15초당 CPU 0.005초, 이벤트 733건 → 9건.

### 재현 시나리오 (수정 사항 건드릴 때 다시 돌려볼 것)

번들을 직접 실행해 stderr를 보면서, 감시 폴더에 다음을 넣고 유휴 CPU와 로그 증가를 본다.

1. NFD 이름 파일 하나 → NFC 변환, 이후 유휴 CPU ~0, 로그 증가 0
2. NFD 이름이 든 폴더를 통째로 이동 → 안쪽 파일까지 NFC 변환
3. 읽기 전용 하위 폴더 안의 NFD 파일 → 로그 1줄만 남고 이후 건너뜀(폭주 없음)
